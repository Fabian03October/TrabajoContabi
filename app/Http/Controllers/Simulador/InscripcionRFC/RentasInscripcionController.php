...
class RentasInscripcionController extends Controller
{
    ...
    public function rentas()
     {
        $actividades = Actividade::where('categoria', 'Rentas')->get(); 
        return view('contaito.inscripcion.Rentas.rentas', compact('actividades')); 
     }

     //funcion para mandar a la vita de porcentajes las activiades seleccionadas
     public function porcientoRentas(Request $request)
     {
        ...
        //  dd($this->numeros);
         ...
         return view('contaito.inscripcion.Rentas.porcentajes', compact('actividades'));
     }

     //Funcion para procesar los porcentajes y mandar a la vista de obligaciones
     public function procesarRentas(Request $request)
    {
        ...
        return view('contaito.inscripcion.Rentas.mostrarObligacionesProfe', compact('actividades', 'obligaciones', 'regimen'));
    }


    public function inscribirRentas(Request $request)
    {
        ...
        if (empty($filteredActivities)) {
            return redirect()->route('inscripcion.procesarRentas')->withErrors('No se encontraron actividades para procesar.');
        }
        // dd(Auth::user()->id);
