...
class ExplotacionInscripController extends Controller
{
    ...
    public function Explotacion()
     {
        $actividades = Actividade::where('categoria', 'Explotacion')->get(); 
        return view('contaito.inscripcion.Explotacion.profesiona', compact('actividades')); 
     }

     //funcion para mandar a la vita de porcentajes las activiades seleccionadas
     public function porcientoExplotacion(Request $request)
     {
        ...
        //  dd($this->numeros);
         ...
         return view('contaito.inscripcion.Explotacion.porcentajes', compact('actividades'));
     }

     //Funcion para procesar los porcentajes y mandar a la vista de obligaciones
     public function procesarExplotacion(Request $request)
    {
        ...
        return view('contaito.inscripcion.Explotacion.mostrarObligacionesProfe', compact('actividades', 'obligaciones', 'regimen'));
    }


    public function inscribirExplotacion(Request $request)
    {
        ...
        if (empty($filteredActivities)) {
            return redirect()->route('inscripcion.procesarExplotacion')->withErrors('No se encontraron actividades para procesar.');
        }
        // dd(Auth::user()->id);
