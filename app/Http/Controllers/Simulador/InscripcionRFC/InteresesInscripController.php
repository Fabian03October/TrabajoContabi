...
class InteresesInscripController extends Controller
{
    ...
    {
        // Redirigir a la ruta inscripcion.porcientoIntereses
        return redirect()->route('inscripcion.porcientoIntereses'); 
    }

     //funcion para mandar a la vita de porcentajes las activiades seleccionadas
     public function porcientoIntereses()
    {
        // Obtener las actividades de la categoría "Intereses" desde la base de datos
        $actividades = Actividade::where('categoria', 'Intereses')->get();

        // Redirigir a la vista con las actividades seleccionadas
        return view('contaito.inscripcion.Intereses.porcentajes', compact('actividades'));
    }

     //Funcion para procesar los porcentajes y mandar a la vista de obligaciones
     public function procesarIntereses(Request $request)
    {
        ...
        return view('contaito.inscripcion.Intereses.mostrarObligacionesProfe', compact('actividades', 'obligaciones', 'regimen'));
    }


    public function inscribirIntereses(Request $request)
    {
